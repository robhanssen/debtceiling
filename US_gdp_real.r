...
gdp <- read_csv("sources/gdp_real.csv") %>%
        janitor::clean_names() %>%
...
gdp_allyears <-
    ...
    ggplot() +
    aes(floordate, gdp, color = party, group = TRUE) +
    geom_line() +
 ...
 ggsave("graphs/us-real-gdp-since-1980.png", width = 6, height = 6, plot = gdp_allyears)
...
gdp2 <- gdp_full_scale +
        ...
        annotate("label", x = as_datetime("2020-01-01"), y = 20000, label = paste0(pct_increase, "\nr.sq = ", scales::pvalue(rsq)))  +
